/*------------------------------------------------------------------------
    File        : myDumbEmailThing.p
    Purpose     : 

    Syntax      : 

    Description : Replaces all emails with the gmailUser+firstname.lastname@gmail.com (or another given domain)

    Author(s)   : michaelzr
    Created     : 3/1/24
    Notes       : Originally written to update fake emails. Adjusted to change all emails.
  ----------------------------------------------------------------------*/
...
define variable gmailUser            as character no-undo.
...
/* **** RE-ENABLING THIS USES THE DOMAIN CHECK TO ONLY UPDATE FAKE EMAIL ADDRESSES ****
...
assign
    gmailUser            = "michaelzrork" // THE +FIRSTNAME.LASTNAME WILL BE ADDED IN THE CODE
    newDomain            = "@gmail.com"
    ...
    // SPECIAL CHARACTER LIST TO BE REMOVED FROM EMAIL ADDRESSES WHEN PULLING CUSTOMER NAMES AS THE USERNAME; THIS DOESN'T REMOVE . OR , OR +
...
for each Member no-lock:
    ...
    assign
        ...
        // RE-ENABLE TO USE FAKE EMAIL CHECK
        // firstHalf       = ""
    ...
    // RE-ENABLE TO ONLY UPDATE PRIMARY GUARDIAN AND ACCOUNT EMAILS; LEAVE DISABLED TO UPDATE Member EMAILS AS WELL
    // if Member.PrimaryEmailAddress = "" and isPrimary = false then next Member-loop.
    
    /* RE-ENABLE TO RESTORE FAKE EMAIL DOMAIN CHECK
    ...
    */
    ...
    for each EmailContact where EmailContact.PrimaryEmailAddress = false and EmailContact.MemberLinkID = Member.ID:
        assign
            ...
            newEmailAddress = "".
        /* DISABLED TO DELETE ALL SECONDARY EMAIL ADDRESSES; RE-ENABLE TO CHECK AGAINST FAKE EMAIL LIST
        ...
        if (EmailContact.EmailAddress <> "" and (lookup(substring(EmailContact.EmailAddress,atPosition),emailDomainList) > 0 or lookup(EmailContact.EmailAddress,fullEmailList) > 0 or lookup(firstHalf,firstHalfList) > 0)) or EmailContact.EmailAddress = "" then */
        run deleteSecondaryEmail(EmailContact.ID).
    end.
end. 

// CREATE LOG FILE
do ixLog = 1 to inpfile-num:
    if search(sessiontemp() + "myDumbEmailThingLog" + string(ixLog) + ".csv") <> ? then 
        SaveFileToDocuments(sessiontemp() + "myDumbEmailThingLog" + string(ixLog) + ".csv", "\Reports\", "", no, yes, yes, "Report").  
end.
...
procedure put-stream:
    ...
    inpfile-loc = sessiontemp() + "myDumbEmailThingLog" + string(inpfile-num) + ".csv".
...
procedure ActivityLog:
    ...
    do for BufActivityLog transaction:
        ...
        assign
            BufActivityLog.SourceProgram = "myDumbEmailThing.r"
            BufActivityLog.LogDate       = today
            BufActivityLog.LogTime       = time
            BufActivityLog.UserName      = "SYSTEM"
            BufActivityLog.Detail1       = "Replaced all emails with " + gmailUser + "+firstname.lastname@gmail.com (or alternate domain)"
            bufActivityLog.Detail2       = "Check Document Center for myDumbEmailThingLog to see log file"
