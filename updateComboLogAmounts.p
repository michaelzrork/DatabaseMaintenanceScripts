...
&global-define ProgramName "UpdateComboLogAmounts" /* PRINTS IN AUDIT LOG AND USED FOR LOGFILE NAME, DO NOT INCLUDE THE .p OR .r */
&global-define ProgramDescription "Update Combo Log for Records with a Zero Dollar Amount"  /* PRINTS IN AUDIT LOG WHEN INCLUDED AS INPUT PARAMETER */
...
using Business.Library.Model.DAO.*.
using Business.Library.MTComboLogSplit.*.
...
define variable useResourceCode as character           no-undo.
define variable ComboLogRecord  as MTComboLogRecordDAO no-undo. 
...
run ActivityLog({&ProgramDescription},"Program in Progress","Number of Records Found So Far: " + addCommas(numRecs),"","","").

for each MTComboLog no-lock where MTComboLog.ResourceType = "Labor" /*and MTComboLog.ResourceCode = useResourceCode*/
    and MTComboLog.Amount = 0 /*and MTComboLog.BeginDate le useDate*/: 
    /* UPDATE AUDIT LOG WITH LAST TABLE/ID AND CURRENT RECORD COUNTS */ 
    assign 
        cLastID   = getString(string(MTComboLog.ID)) // REPLACE 0 WITH TABLENAME.ID 
        LastTable = "MTComboLog". // REPLACE <TABLE NAME> WITH THE TABLE NAME
    run UpdateActivityLog({&ProgramDescription},"Program in Progress; Last Record ID - " + getString(lastTable) + ": " + getString(cLastID),"Number of Records Found So Far: " + addCommas(numRecs),"","").
    ComboLogRecord = DAO:Instance:Factory:MTComboLog:GetByID(MTComboLog.id).
    ComboLogRecord:UpdateLog(). 
    numRecs = numRecs + 1. 
end.
...
run UpdateActivityLog({&ProgramDescription},"Program is Complete","Number of Records Found: " + addCommas(numRecs),"","").
