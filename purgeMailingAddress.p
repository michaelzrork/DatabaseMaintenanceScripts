/*------------------------------------------------------------------------
    File        : purgeSAAddress.p
    Purpose     : 

    Syntax      : 

    Description : Purge all MailingAddress Records
...
for each AccountAddress no-lock:
    run purgeSAHouseholdAddress(AccountAddress.ID).
end.
...
procedure ActivityLog:
    ...
    do for bufActivityLog transaction:
        ...
        assign
            bufActivityLog.SourceProgram = "purgeSAAddress.r"
            bufActivityLog.UserName      = "SYSTEM"
            bufActivityLog.LogDate       = today
            bufActivityLog.LogTime       = time
            bufActivityLog.Detail1       = "Purge all MailingAddress Records"
