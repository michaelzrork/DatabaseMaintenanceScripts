...
for each Charge no-lock where Charge.CloneID <> 0 and lookup(Charge.RecordStatus,"Active,Charge,NoCharge,Reset") > 0 by Charge.ID:
    if Charge.RecordStatus = "Reset" and Charge.PreviousStatus = "Reset" then next fee-loop.
...
procedure checkForDuplicateFee:
    ...
    assign 
        hasDuplicate = false.
    for first bufCharge no-lock where bufCharge.ID <> inpID and bufCharge.ParentRecord = feeParentID and bufCharge.CloneID = feeCloneID and lookup(bufCharge.RecordStatus,"Charge,NoCharge,Reset") > 0 and bufCharge.ReceiptNumber > Charge.ReceiptNumber and bufCharge.LogDate ge Charge.LogDate by bufCharge.ID:
        assign 
            hasDuplicate = true.
        run updateFeeStatus(Charge.ID,bufCharge.ReceiptNumber,"Reset","Reset",bufCharge.RecordStatus,bufCharge.PreviousStatus,bufCharge.LogDate).
    end.
    if not hasDuplicate and Charge.RecordStatus = "Reset" then run updateFeeStatus(Charge.ID,0,"Reset","Reset","Reset Fee Not Found","Reset Fee Not Found").
...
procedure updateFeeStatus:
    ...
    define input parameter resetLogDate as date no-undo.
    define buffer bufCharge for Charge.
