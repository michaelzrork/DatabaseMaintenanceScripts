/*------------------------------------------------------------------------
    File        : findSplitCreditCardPayments.p
    Purpose     : 
...
do ixLog = 1 to inpfile-num:
    if search(sessiontemp() + "findSplitCreditCardPaymentsLog" + "_" + replace(string(logfileDate),"/","-") + "_" + string(logfileTime) + "_" + string(ixLog) + ".csv") <> ? then 
        SaveFileToDocuments(sessiontemp() + "findSplitCreditCardPaymentsLog" + "_" + replace(string(logfileDate),"/","-") + "_" + string(logfileTime) + "_" + string(ixLog) + ".csv", "\Reports\", "", no, yes, yes, "Report").  
end.
...
procedure put-stream:
    ...
    inpfile-loc = sessiontemp() + "findSplitCreditCardPaymentsLog" + "_" + replace(string(logfileDate),"/","-") + "_" + string(logfileTime) + "_" + string(inpfile-num) + ".csv".
...
procedure ActivityLog:
    ...
    do for BufActivityLog transaction:
        ...
        assign
            BufActivityLog.SourceProgram = "findSplitCreditCardPayments.r"
            BufActivityLog.LogDate       = today
            BufActivityLog.LogTime       = time
            BufActivityLog.UserName      = "SYSTEM"
            BufActivityLog.Detail1       = "Find Split Credit Card Payments"
            BufActivityLog.Detail2       = "Check Document Center for findSplitCreditCardPaymentsLog for a log of Records Changed"
